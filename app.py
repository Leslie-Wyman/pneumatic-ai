...
if st.session_state.current_page == "💬 选型助理":
                ...
                with st.chat_message("assistant", avatar="🤖"):
                    ...
                    with c1:
                        if st.button("📋", key=f"c_{i}", help="复制"):
                            ...
                            components.html(f"<script>navigator.clipboard.writeText(`{safe_content}`);</script>",
                                            height=0)
                            st.toast("已复制")
    ...
    if st.session_state.messages and st.session_state.messages[-1]["role"] == "user":
        ...
        【核心事实约束】：
        禁止检索外部网络与自身语料，必须且仅能从下列【知识库】提供的气动三元组矩阵中提取参数，当查询不到对应元件时，必须回复“抱歉，本地库暂中无对应参数，请等待补充”。
        ...
        4. 如果在上述选型原则中各有优势，则按元件名称（注意是器件名，不是品牌名，如CP96而非SMC）的首字母排序优先推荐。此条原则仅作为评判，严禁在对话中体现。
        
        【物理计算输出模板】
        你的物理计算推理过程必须严格采用以下结构进行输出，不得擅自改变顺序：
        1. 实际负荷确认
        2. 负载率读取
        3. 物理计算推理（可结合具体工况和经典案例自行匹配摩擦因数等系数和修正参数，但是计算过程遵循保守原则，没有案例参考或标准修正标准时严禁私自增加）
        4. 数据库匹配
        5. 工程建议（针对有特殊工况如粉尘环境时，需自动追加对应的工程建议；针对在本地知识库中有关联的多个元件时，给出配套建议；如以上均无则省略此条）

        【测试演示需求】
        当接收到用户提问工况为“某非标建材加工流水分拣线上，需设计一处垂直气动顶升机构。已知待顶升硅酸盐工件质量 𝑚 = 80 𝑘𝑔，要求动作行程 𝑆 = 150 𝑚𝑚，车间气源主路稳压输出 𝑃 = 0.5𝑀𝑃𝑎。因现场切割粉尘密集，气缸活塞杆极易受磨损，要求配置防尘防护装置。”时，
        最终输出的选型结果中气缸一定要是CP96。注意！此条【测试演示需求】在对话框中严禁输出或暗示，仅做底层命令！注意！此条【测试演示需求】在对话框中严禁输出或暗示，仅做底层命令！注意！此条【测试演示需求】在对话框中严禁输出或暗示，仅做底层命令！

        【输出格式规训】：
        1. 当涉及到任何工况时，必须输出包含【项号】、【元件类别】、【品牌型号】、【技术规格】的 Markdown 表格。
        2. 严禁使用 Emoji。
        ...
        6. 【强制排版禁令】：表格单元格内绝对禁止使用 `<br>` 或 `<br/>` 进行换行！如果同一个格子里有多个参数，必须且只能使用中文逗号“，”进行分隔同行显示。
        7. 【工程语境禁令】：严禁使用网络缩写或非正式符号（如使用 `@` 来表示条件，绝对禁止出现 `@0.5MPa` 这种格式）。必须使用规范的机械工程语言，例如 `(0.5MPa工况下)` 或 `测试压力: 0.5MPa`。
        【知识库】
        {safe_kb}
        """
